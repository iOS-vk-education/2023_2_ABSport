...
final class ScheduleViewModelImpl: ScheduleViewModel {
    ...
    func areDaysFromDifferentMonths(date1: Date, date2: Date) -> Bool {
        calendarManager.areDaysFromDifferentMonths(date1: date1, date2: date2)
    }
    ...
    func showOrDismissCalendar() {
        state.isCalendarShowing.toggle()
    }
    ...
    func updateCurrentDay(tappedDay day: Date) {
        if calendarManager.areDaysFromDifferentMonths(date1: day, date2: state.currentDate) {
    ...
    func getCurrentMonthDays() {
        state.currentMonthDates = calendarManager.getCurrentMonth(withThisDate: state.currentDate)
    ...
    func getCurrentMonthDaysReservations() {
        ...
        state.areReservationsFetched = false
        // TODO: - delete just mock
        ...
        ReservationManager.shared.getAllReservations { [weak self] dictionary in
            ...
            self.state.areReservationsFetched = true
        }
    }
