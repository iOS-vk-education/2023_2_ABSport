...
//                    }))
            ...
            LazyVGrid(columns: columns, spacing: 7) {
                ForEach(viewModel.state.currentWeek, id: \.self) { day in
                    Text(viewModel.formate(date: day, toType: .day))
                        .foregroundColor(viewModel.isSameDay(date1: viewModel.state.currentDate, date2: day)
                                         ? .white
                                         : Color("TextColor"))
                        .background(
                            Circle()
    ...
    func timeTableView() -> some View {
        ...
        VStack {
            if viewModel.state.areTimeSlotsLoaded {
                Text("Выберите время")
                    .foregroundStyle(Color("TextColor"))
                    .font(.system(size: 16, weight: .semibold))
                    .padding(.top, 38)
            ...
            Text("Выбрать")
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8)
                        .foregroundColor(viewModel.state.isChooseButtonDisabled
                                         ? Color("DarkGrayColor")
                                         : Color("BlueButtonColor"))
                )
        })
        .disabled(viewModel.state.isChooseButtonDisabled)
        .padding(.bottom, 27)
    }
